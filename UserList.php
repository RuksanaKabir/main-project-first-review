...
<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST")	
		{
			$value=$_POST['value'];
			$sql="select * from user where (name='$value' or email='$value' or phone='$value') and status=0"; //echo $sql;
		...
		{
			$sql="select * from user where status=0"; //echo $sql;
			$result=mysql_query($sql);
			$row=mysql_fetch_array($result);
			$m = mysql_num_rows($result);
			if($m < 1) { $msg ="No results found..!";}
...
<body>
<center>
  <h1>User List -Request for Login </h1>
	  <form  name="form1" method="post" >
		  <table width="335" height="78">
			<tr>
			  ...
			  <a href="UserList.php">Clear search</a></label></td>
...
<?php if(!isset($msg)) { ?>
  ...
  <tr>
	...
	<td><a href="approveUser.php?id=<?php echo $r["id"]; ?>"> Approve </a></td>
  </tr>
  <?php } ?>
</table>
<?php } ?>
